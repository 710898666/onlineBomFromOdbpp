...
def get_parser_by_extension(file_name, config, logger):
    ...
    elif lower_name.endswith('.tgz') or lower_name.endswith('.tar.gz'):
        return get_odbpp_parser(file_name, config, logger)
...
def get_odbpp_parser(file_name, config, logger):
    from .odbpp import OdbPlusPlusParser
    return OdbPlusPlusParser(file_name, config, logger)
